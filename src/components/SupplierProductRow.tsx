...
type Row = { id: string; price: number; moq: number; product: Product };
...
export function SupplierProductRow({ row, onChange }: { row: Row; onChange: () => void }) {
  ...
  async function save() {
    setBusy(true);
    try {
      await editOffer(row.id, Number(price) || 0, Number(moq) || 1);
      setEditing(false);
      onChange();
    } finally {
      setBusy(false);
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-2xl border border-border p-3">
      ...
      {editing ? (
        <>
          <button onClick={save} disabled={busy} className="rounded-xl p-2 text-emerald-600 hover:bg-emerald-50" aria-label="حفظ">
            <Check className="h-4 w-4" />
          </button>
          <button onClick={() => setEditing(false)} className="rounded-xl p-2 text-muted-foreground hover:bg-secondary" aria-label="إلغاء">
            <X className="h-4 w-4" />
          </button>
        </>
      ) : (
        <>
          ...
          <button onClick={() => setEditing(true)} className="rounded-xl p-2 text-muted-foreground hover:bg-secondary" aria-label="تعديل">
            <Pencil className="h-4 w-4" />
          </button>
          <button onClick={remove} disabled={busy} className="rounded-xl p-2 text-rose-600 hover:bg-rose-50" aria-label="حذف">
